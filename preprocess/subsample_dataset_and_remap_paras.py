...
from preprocess.lib import read_jsonl, write_jsonl, find_matching_paragraph_text
...
def main(args):
    ...
    if args.set_name == "test":
        dev_file_path = os.path.join("data", "processed_data", args.dataset_name, "dev_subsampled.jsonl")
        avoid_question_ids_file_path = dev_file_path if os.path.exists(dev_file_path) else None
        sample_size = 500
        
    elif args.set_name == "dev_diff_size":
        avoid_question_ids_file_path = os.path.join("data", "processed_data", args.dataset_name, "test_subsampled.jsonl")
        sample_size = args.sample_size
        
    else:
        sample_size = 100

    input_file_path = os.path.join("data", "processed_data", args.dataset_name, "dev.jsonl")
    ...
    if args.set_name == "dev_diff_size":
        output_file_path = os.path.join("data", "processed_data", args.dataset_name, f"dev_{args.sample_size}_subsampled.jsonl")
        write_jsonl(instances, output_file_path)
    else:
        output_file_path = os.path.join("data", "processed_data", args.dataset_name, f"{args.set_name}_subsampled.jsonl")
        write_jsonl(instances, output_file_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_name", 
        type=str, choices=("hotpotqa", "2wikimultihopqa", "musique", 'nq', 'trivia', 'squad'),
        help="dataset name."
    )
    parser.add_argument(
        "--set_name", 
        type=str, choices=("dev", "test", "dev_diff_size"), 
        help="set name."
    )
    parser.add_argument(
        "--sample_size", 
